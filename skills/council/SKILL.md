...
allowed-tools:
  ...
  - "Bash(python3 ${SKILL_ROOT}/scripts/api_server.py:*)"
  - "Read(**/*.py)"
  - "Read(**/*.ts)"
  - "Read(**/*.js)"
  - "Read(**/*.tsx)"
...
The Council skill includes an HTTP API server for programmatic access and integration.
...
python3 ${SKILL_ROOT}/scripts/api_server.py --port 8080
```
...
| GET | `/models` | List available models and circuit breaker status |
| POST | `/council` | Run deliberation (blocking, returns final result) |
| POST | `/council/stream` | Run with Server-Sent Events streaming |
...
```json
{
  "query": "What database should we use for real-time chat?",
...
```json
{
  "session_id": "council-1234567890",
  "mode": "consensus",
  "answer": "The council recommends...",
  "confidence": 0.89,
  "raw_confidence": 0.89,
  "convergence_score": 0.944,
  "rounds_completed": 2,
  "degradation_level": "full",
  "duration_ms": 45230,
  "models_responded": ["claude", "gemini", "codex"]
}
```
...
```bash
curl -N -X POST http://localhost:8080/council/stream \
  -H "Content-Type: application/json" \
  -d '{"query": "Should we use TypeScript?", "mode": "vote"}'
```

Events stream as Server-Sent Events:
```
data: {"type": "status", "stage": 0, "msg": "Starting vote session..."}
data: {"type": "vote_cast", "model": "claude", "vote": "yes", "confidence": 0.92}
data: {"type": "vote_cast", "model": "gemini", "vote": "yes", "confidence": 0.88}
data: {"type": "vote_cast", "model": "codex", "vote": "yes", "confidence": 0.95}
data: {"type": "result", "data": {"answer": "...", "confidence": 0.91}}
data: {"type": "done"}
```
...
```javascript
const ws = new WebSocket('ws://localhost:8080/council/ws');
...
response = requests.post('http://localhost:8080/council', json={
    'query': 'What is the best caching strategy?',
    'mode': 'consensus',
    'max_rounds': 2
})
